...
import type { Project, Image as PrismaImage } from "@prisma/client";
...
export default function EditProjectForm({ project }: EditProjectFormProps) {
  ...
  const [state, formAction, isPending] = useActionState(updateWithId, {} as ProjectFormState);
  
  const initialCategoryIsPreset = PRESET_CATEGORIES.includes(project.category);
  const [selectedCategory, setSelectedCategory] = useState(
    initialCategoryIsPreset ? project.category : "__custom__"
  );
  
  const [imageIds, setImageIds] = useState<string[]>(project.images.map(img => img.id));
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-semibold text-black tracking-tight">Edit Project</h1>
          <p className="text-[#888] mt-1">Update details for the portfolio item.</p>
        </div>
      </div>
      ...
      >
        <div className="p-6 rounded-xl border border-[#d6d6d6] bg-white space-y-6">
          ...
          {isCustom && (
            <div className="space-y-1">
              ...
              <input
                ...
                defaultValue={!initialCategoryIsPreset ? project.category : ""}
          ...
          <div className="flex items-center gap-3 py-1">
            <input 
              ...
              className="w-4 h-4 rounded border-[#999] bg-[#f6f6f6] accent-black cursor-pointer" 
            />
        ...
        <div className="p-6 rounded-xl border border-[#d6d6d6] bg-white space-y-3">
          ...
          <ImageCropUploader 
            ...
            initialImages={project.images.map(img => ({ id: img.id, url: img.url }))} 
          />
